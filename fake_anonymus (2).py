...
# Admin ID
id_bahar = '5970933995'  # Replace with your actual admin chat ID
...
def view_messages(message):
    try:
        if str(message.chat.id) == id_bahar:
            if user_messages:
                for idx, (user_id, user_name, text) in enumerate(user_messages):
                    ...
                    markup.add(btn)
                    bot.send_message(id_bahar, f'Message from @{user_name} ({user_id}): {text}', reply_markup=markup)
            else:
                bot.send_message(id_bahar, "No messages to display.")
        else:
...
@bot.message_handler(func=lambda message: message.chat.id != int(id_bahar))
...
def handle_response(call):
    try:
        if str(call.message.chat.id) == id_bahar:
            idx = int(call.data.split('_')[1])
            user_id, user_name, text = user_messages[idx]
            msg = bot.send_message(id_bahar, "Please type your response:")
...
def process_response(message, user_id, user_name, idx):
    try:
        ...
        bot.send_message(id_bahar, f'Response sent to @{user_name} ({user_id}).')
...
def clear_messages(message):
    try:
        if str(message.chat.id) == id_bahar:
            user_messages.clear()
            bot.send_message(id_bahar, "All messages have been cleared.")
        else:
            bot.reply_to(message, "This command is for admin only.")
    except Exception as e:
